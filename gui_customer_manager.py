...
class CustomerListWindow(tk.Toplevel):
    def __init__(self, parent):
        ...
        # Main column containing all content
    ...
    def _setup_form_frame(self):
        ...
        form_frame.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="ew")  # Expand horizontally
        form_frame.columnconfigure(1, weight=1)  # Allow entry fields to expand within the form
        ...
        form_button_frame = ttk.Frame(form_frame)  # Frame for form buttons
        form_button_frame.grid(row=3, column=0, columnspan=2, pady=(10, 5))  # Center buttons
    ...
    def _setup_search_frame(self):
        ...
        search_frame.grid(row=1, column=0, sticky="ew")  # Expand horizontally
    ...
    def _setup_customer_list_tree(self):
        ...
        list_frame.grid(row=2, column=0, padx=10, pady=5, sticky="nsew")  # Expand in all directions
        list_frame.rowconfigure(0, weight=1)  # Make the row containing the treeview expandable
        list_frame.columnconfigure(0, weight=1)  # Make the column containing the treeview expandable
        ...
        self.customer_tree.grid(row=0, column=0, sticky="nsew")  # Treeview fills its cell
        ...
        customer_scrollbar.grid(row=0, column=1, sticky="ns")  # Scrollbar sticks to north-south
    ...
    def _setup_purchase_history_tree(self):
        ...
        history_frame.grid(row=3, column=0, padx=10, pady=5, sticky="nsew")  # Expand in all directions
        history_frame.rowconfigure(0, weight=1)  # Make the row containing the treeview expandable
        history_frame.columnconfigure(0, weight=1)  # Make the column containing the treeview expandable
        ...
        self.purchase_history_tree = ttk.Treeview(history_frame, columns=self.history_columns, show="headings",
                                                  selectmode="none")
        # ... (rest of purchase_history_tree setup) ...
        ...
        self.purchase_history_tree.grid(row=0, column=0, sticky="nsew")  # Treeview fills its cell
        ...
        history_scrollbar.grid(row=0, column=1, sticky="ns")  # Scrollbar sticks to north-south

    def _setup_bottom_buttons(self):
        """Sets up the main action buttons at the bottom."""
        bottom_button_frame = ttk.Frame(self)
        bottom_button_frame.grid(row=4, column=0, pady=10)  # Centered by default if no column weights
    ...
    # ... (rest of the methods remain the same as in your last provided version of gui_customer_manager.py) ...
    # Make sure to include all methods:
    # filter_customer_list, clear_form, on_customer_select, _populate_purchase_history,
    # save_or_update_customer, delete_selected_customer, populate_customer_list,
    ...
    def delete_selected_customer(self):
        ...
        logging.warning(
            f"Confirmation requested for deleting customer '{customer_name}' (ID: {self.selected_customer_id}).")
    ...
    def populate_customer_list(self, search_term=""):
        ...
        for customer_data in filtered_customers:
            ...
            self.customer_tree.insert("", tk.END, iid=cust_id,
                                      values=(seq_counter, name, display_contact, display_address))
